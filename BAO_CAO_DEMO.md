...
### 3.4 Dữ liệu này phục vụ app WiseTravel chính thức thế nào
Đồ án tạo ra hai thành phần sẵn sàng đưa vào sản phẩm:

1. **Kho địa điểm** (bảng `pois`) — app chính thức truy vấn trực tiếp để hiển thị và tìm kiếm
   địa điểm theo loại hình, giờ mở cửa, mức giá, tọa độ.
2. **Mô-đun cảm xúc** — dùng model đã chọn (**ViSoBERT**) để tự động chấm cảm xúc review người
   dùng và gộp thành điểm hài lòng cho từng địa điểm.

Nói cách khác, đồ án đóng vai trò **tầng dữ liệu (backend data)** của app: chuẩn bị dữ liệu địa
điểm đã kiểm chứng chất lượng và mô hình phân tích cảm xúc, để phần giao diện app chính thức chỉ
việc đọc và hiển thị.
